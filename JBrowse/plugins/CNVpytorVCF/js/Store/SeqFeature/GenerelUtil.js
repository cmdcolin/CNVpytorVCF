...
function fit_normal(bins, dist_p) {
  ...
  var sum_xy = bins.reduce(function (r, a, i) {
    return r + a * dist_p[i];
  }, 0);
  ...
  var sigma = Math.sqrt(
    dist_p.reduce((r, a, i) => {
      return r + a * Math.pow(bins[i] - mean, 2);
    }) / mean,
  );
  var area = dist_p.reduce((r, a, i) => {
...
function Gaussian([a, x0, sigma]) {
  ...
  return x =>
    (a * Math.exp(-Math.pow(x - x0, 2) / (2 * Math.pow(sigma, 2)))) /
